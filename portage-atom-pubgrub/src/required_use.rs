...
//!
//! This mirrors `portage_metadata::RequiredUseExpr` but uses interned flag names
//! (the type the solver works in) instead of `String`, so the solver crate stays
//! decoupled from the md5-cache parser: the caller translates the parsed
//! metadata grammar into this type when it builds [`PackageVersions`], exactly as
//! it already turns metadata strings into `DepEntry` / [`PackageDeps`].
//!
