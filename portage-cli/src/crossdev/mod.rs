...
/// path. Currently only reachable via [`source_repo`] for a
/// non-`gentoo` [`CrossTarget::source_repo`] (none exist today — Darwin's
/// toolchain packages are deployed straight into `::gentoo`, see
/// `dev/sync-to-gentoo.sh` — but the alias mechanism stays repo-name
/// generic).
///
...
/// in — currently always `main_repo` ([`CrossTarget::source_repo`] is
/// always `"gentoo"`), but kept repo-name generic via [`named_repo`] for
/// a future non-`gentoo` source (no hardcoded default path in that case:
/// unlike `gentoo`, there's no portage-wide well-known location, so a
/// missing entry would be a real configuration error).
