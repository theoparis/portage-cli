...
fn list_all_gcc_profiles(globals: &Cli) -> Result<BTreeMap<String, Vec<GccProfile>>> {
    ...
    let prefix_base_dir = gcc_env_d_dir(globals);
    if prefix_base_dir.is_dir() {
        collect_profiles(&prefix_base_dir, &mut profiles_by_target, false)?;
    }
    ...
    if is_prefix_context {
        let system_dir = Utf8PathBuf::from("/etc/env.d/gcc");
        if system_dir.is_dir() {
            collect_profiles(&system_dir, &mut profiles_by_target, true)?;
        }
    }
...
fn collect_profiles(
    ...
    profiles_by_target: &mut BTreeMap<String, Vec<GccProfile>>,
    is_host: bool,
) -> Result<()> {
